...
def Train():
    ...
    pretrained_param_dict = torch.load(pretrained_model_path)
    model_dict = Encoder.state_dict()
    state_dict = {k: v for k, v in pretrained_param_dict.items() if k in model_dict.keys()}
    model_dict.update(state_dict)
    Encoder.load_state_dict(model_dict)
    for p in Encoder.lstm.parameters():
        p.requires_grad = False

    myadam_encode = torch.optim.Adam(filter(lambda p: p.requires_grad, Encoder.parameters()), lr=mylr)
    mse_loss = nn.MSELoss()
    ...
    torch.save(Encoder.state_dict(), './model/auto_encoder_method_%s.pth' % time.time())
    pd.DataFrame(train_avg_loss_list).to_csv('ae_train_avg_loss_list.csv')
    pd.DataFrame(test_avg_loss_list).to_csv('ae_test_avg_loss_list.csv')
